...
void addressconfigure(void)
{
    ...
    instance_anchaddr = (is_switch_on(TA_SW1_7) << 1) + is_switch_on(TA_SW1_8);
    ipc.anchorAddress = anchorAddressList[instance_anchaddr];
    ...
    ipc.anchorPollMask = getanchorpollmask((is_switch_on(TA_SW1_7) << 1) + is_switch_on(TA_SW1_8));
...
int main(void)
{
...
#else
                memcpy(&dataseq[2], (const uint8 *) " TAG POLL   ", 12);
#endif
                writetoLCD( 40, 1, dataseq); //send some data
    ...
    while(1)
    {
        ...
        if(instancenewrange())
        {
            ...
            if(instance_mode == TAG)
            {
                sprintf((char*)&dataseq[0], "t%04x a%04x %4.2f %4d %04d %d %d", taddr, aaddr, range_result, l, instance_get_respPSC(), txl, rxl);
            }
            else
            {
                sprintf((char*)&dataseq[0], "a%04x t%04x %4.2f %4d %04d %d %d", aaddr, taddr, range_result, l, (dwt_read16bitoffsetreg(0x10, 2) >> 4), txl, rxl);
            }
...
#else
                        memcpy(&dataseq[2], (const uint8 *) "   TAG POLL     ", 16);
#endif
                        writetoLCD( 40, 1, dataseq); //send some data
                        sprintf((char*)&dataseq[0], "%llX", instance_get_addr());
                        writetoLCD( 16, 1, dataseq); //send some data
