...
// -------------------------------------------------------------------------------------------------------------------
//
// function to add user payload to the message/frame
//
// -------------------------------------------------------------------------------------------------------------------
//
...
int adduserpayload(instance_data_t *inst, int offset)                // ADD USER PAYLOAD
{
    int i,j ;
    char c ;
	int flen = 0;

    for (i = offset, j=0 ; i < MAX_USER_PAYLOAD_STRING ; i++, j++)
    {
        inst->msg.messageData[i] = c = inst->payload.payloadString[j] ;
        if (c == 0) break;
    }
    flen += (i - offset) ;

    if (inst->payload.countAppend)
    {
        flen += sprintf_s((char*)&inst->msg.messageData[i],9,"%08i",inst->payloadCounter++) ;
    }
    #if (DECA_SUPPORT_SOUNDING==1)
    #if DECA_ACCUM_LOG_SUPPORT==1
    // both copy loop and sprintf above null terminate the string so can use directly
    instancestatusaccumlog(0, "TXD", (char*) &inst->msg.messageData[offset]) ;
    #endif
    #endif
...
int destaddress(instance_data_t *inst)
{
    ...
    while(getnext)
    {
        ...
        {
            memcpy(&inst->msg.destAddr[0], &inst->payload.anchorAddressList[inst->anchorListIndex], ADDR_BYTE_SIZE);
            getnext = 0;
        }
...
//#endif
        ...
        case TA_PAUSED :    // (idle) state when application pause button is pressed
            ...
            case 1 : // entering pause state
				...
				if(inst->deviceissleeping == 1) //wake up
				{
					uint8 buffer[1500];
					inst->deviceissleeping = 0; //this is to start polling device status register (after it wakes up)
		...
		case TA_SLEEP_DONE :
			{
				...
				if(inst->tagDeepSleep)
				{
					...
					if(dwt_spicswakeup(buffer, 1500) == DWT_ERROR) //1000 takes 400us, 1300 takes 520us, 1500 takes 600us (SPI @ 20MHz)
					{
						...
						//if(dwt_spicswakeup(buffer, 1500))
							//printf("FAILED to WAKE UP 2\n");
					}
					...
					//set EUI as it will not be preserved unless the EUI is programmed and loaded from OTP
					/*if((inst->mode == TAG) || (inst->mode == TAG_TDOA))
					{
						dwt_setpanid(inst->panid);
						dwt_seteui(inst->eui64);
					}*/
					dwt_entersleepaftertx(0); //clear the auto TX to sleep bit
				}
#endif
			}
            break;
        ...
        case TA_TXFINAL_WAIT_SEND :
            {
				...
				{
					inst->wait4ack = DWT_RESPONSE_EXPECTED;
					dwt_setrxaftertxdelay(80000); //the report will come about 90-110 ms after sending the final so delay RX on (the variation is due to PC)
				}
...
//NOTE - Auto ACK only works if frame filtering is enabled!
                        ...
                        switch(fcode)
                        {
							case RTLS_DEMO_MSG_RNG_INIT:
                            {
								...
								if(inst->mode == TAG_TDOA) //only start ranging with someone if not ranging already
								{
									...
									instancesetreplydelay(messageData[RES_T1] + (messageData[RES_T2] << 8), 0); //set the new timouts/delay times for RX 
		...
		{
			double a1 = convertdevicetimetosecu(aRxT)*1e9 - 150e6;
			double a2 = convertdevicetimetosecu(aTxT)*1e9 - 150e6;
			double b1 = convertdevicetimetosecu(tRxT)*1e9 - 150e6;
			double b2 = convertdevicetimetosecu(tTxT)*1e9 - 150e6;

			fprintf(inst->dwlogdata.accumLogFile,"\naRxT  %08x%08x aTxT %08x%08x\n", (uint32) (aRxT>>32), (uint32) aRxT, (uint32) (aTxT>>32), (uint32) aTxT) ;
			fprintf(inst->dwlogdata.accumLogFile,"\naRxT1  %f aTxT1 %f  ns\n", a1, a2) ;
			fprintf(inst->dwlogdata.accumLogFile,"\ntRxT  %08x%08x tTxT %08x%08x\n", (uint32) (tRxT>>32), (uint32) tRxT, (uint32) (tTxT>>32), (uint32) tTxT) ;
			fprintf(inst->dwlogdata.accumLogFile,"\ntRxT1  %f tTxT1 %f  ns\n", b1, b2) ;
			fprintf(inst->dwlogdata.accumLogFile,"\npollRespRTD  %08x%08x respFinalRTD %08x%08x\n", (uint32) (pollRespRTD>>32), (uint32) pollRespRTD, (uint32) (respFinalRTD>>32), (uint32) respFinalRTD) ;
                                ...
                                if(inst->sendTOFR2Tag == SEND_TOF_REPORT)
                                {
				                        ...
				                        dwt_forcetrxoff(); //disable rx
                                    #endif
									//printf("RX final -> go to TA_TXREPORT_WAIT_SEND\n");
                ...
                case DWT_SIG_RX_TIMEOUT :
					instance_getevent(); //get and clear this event
...
int instance_sleeping(void)
{
    ...
    return instance_data[instance].deviceissleeping;
}
