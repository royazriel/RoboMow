...
void reportTOF(instance_data_t *inst)
{
        ...
        if ((distance < -0.5) || (distance > 20000.000))    // discount any items with error
            return;
		...
		{
			PINFO("Long Term Average = %-3.6f m  (over %i)     Clock Offset = %-2.3f ppm",ltave,inst->longTermRangeCount, inst->clockOffset);
		}
		else
		{
			PINFO("Long Term Average = %-3.6f m  (over %i)",ltave,inst->longTermRangeCount);
		}
        ...
        if(inst->tofcount == RTD_MED_SZ)
        {
			...
			{ // calculate STDEV
                ...
                PINFO("%dms Mean(%i): Dist: %-3.2f m  STDEV(%i): %-3.2f cm ",getmstime()-startTime,RTD_MED_SZ, avg, RTD_STDEV_SZ, stdev*100);
        }
        else
            inst->tofcount++;
...
//#if (DECA_SUPPORT_SOUNDING==1)
        ...
        {
            fprintf(inst->dwlogdata.accumLogFile, "\n%s ToF: %-3.3f ns Dist: %-3.6f m ClockOffset: %-2.3f ppm\n",((inst->mode==TAG) ? "Tag" : "Anchor"), tof*1e9, distance, inst->clockOffset);
        }
#endif
//#endif
...
{
	if(srclen == ADDR_BYTE_SIZE_L)
	{
		sprintf_s(longline,LONG_LINE_LEN,"SrcAdd 0x%02x%02x%02x%02x%02x%02x%02x%02x L(%03i) User payload(%03i) >>> %s <<< ",
			srcadd[7],srcadd[6],srcadd[5],srcadd[4],
			srcadd[3],srcadd[2],srcadd[1],srcadd[0],
			rxlen,
			uplen,
			messageData) ;
	}
	...
	{
		sprintf_s(longline,LONG_LINE_LEN,"SrcAdd 0x%02x%02x L(%03i) User payload(%03i) >>> %s <<< ",
			srcadd[1],srcadd[0],
			rxlen,
			uplen,
			messageData) ;
	}
	copylongline2scrline(statusrep->scr[STAT_LINE_USER_RX1],statusrep->scr[STAT_LINE_USER_RX2]) ;
	
	statusrep->changed = 1 ;
	#if (DECA_SUPPORT_SOUNDING==1)
	#if DECA_ACCUM_LOG_SUPPORT==1
	// log it if logging enabled (rxmsg->userData has been NUL terminated above)
	instancestatusaccumlog(0,"RXD", (char*)messageData) ;
	#endif
	#endif
}
...
void print_rxmessage(event_data_t *dw_event)
{
	...
	uint8 *rxmsgx2 = (uint8*)&dw_event->msgu.rxblinkmsg;
	uint8 *rxmsgx3 = (uint8*)&dw_event->msgu.rxblinkmsgdw;
	...
	fprintf(instance_data[instance].dwlogdata.accumLogFile,"\n");
	fprintf(instance_data[instance].dwlogdata.accumLogFile,"RX BDATA: ");
	for(x=0; x<dw_event->rxLength; x++)
	{
		fprintf(instance_data[instance].dwlogdata.accumLogFile,"%02x", rxmsgx2[x]);
	}
	fprintf(instance_data[instance].dwlogdata.accumLogFile,"\n");
	fprintf(instance_data[instance].dwlogdata.accumLogFile,"RX BDWDATA: ");
	for(x=0; x<dw_event->rxLength; x++)
	{
		fprintf(instance_data[instance].dwlogdata.accumLogFile,"%02x", rxmsgx3[x]);
	}
	fprintf(instance_data[instance].dwlogdata.accumLogFile,"\n");
}
    ...
    case LOG_ALL_ACCUM :
        {
            ...
            case DWT_SIG_RX_NOERR :
				{
					uint32 rxunadj_hi16 = dwt_read16bitoffsetreg(0x15, 0x13);
					uint32 rxunadj_low32 = dwt_read32bitoffsetreg(0x15, 0x9);
					uint32 rxadj_hi16 = (dw_event->timeStamp >> 32) & 0xFF;
					...
					fprintf(instance_data[instance].dwlogdata.accumLogFile," DCR(0), DCI(0), NTH(%04X), T(%04X), RSL(%09.4f), FSL(%09.4f)",
						instance_data[instance].dwlogdata.diag.maxNoise, tempvbat, instance_data[instance].RSL, instance_data[instance].FSL);
...
//						instance_data[instance].devicelogdata.diag.maxNoise, tempvbat, instance_data[instance].RSL, instance_data[instance].FSL, instance_data[instance].devicelogdata.diag.debug1, instance_data[instance].devicelogdata.diag.debug2);
            ...
            case DWT_SIG_RX_SFDTIMEOUT :
				{			
					...
					fprintf(instance_data[instance].dwlogdata.accumLogFile,"\n");
					fprintf(instance_data[instance].dwlogdata.accumLogFile,"\nRX E%c WInd(0735), PSC(%04i), CE(%04i), RSE(%04i), HE(%04i), STO(%04i), FFE(%04i), T(%04X), RSL(%09.4f), FSL(%09.4f)",
                        errorType[errorFlag], // single character from char array defined above
                        instance_data[instance].dwlogdata.diag.rxPreamCount,  
                        itemp.CRCB, itemp.RSL, itemp.PHE, itemp.SFDTO, itemp.ARFE, tempvbat, instance_data[instance].RSL, instance_data[instance].FSL) ;
				}
                break ;
#endif
            }
