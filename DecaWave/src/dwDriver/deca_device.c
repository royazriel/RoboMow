...
//    configure PLL2/RF PLL block CFG
	dwt_writetodevice(FS_CTRL_ID, FS_PLLCFG_OFFSET, 5,
			&pll2_config[chan_idx[chan]][0]);   //alliv rewrite to 32bit + 8 bit
	...
	if (use_otpconfigvalues & DWT_LOADANTDLY) {
		...
		dwt_setrxantennadelay(
				((dw1000local.antennaDly >> (16 * prfIndex)) & 0xFFFF) >> 1);
...
void dwt_readdignostics(dwt_rxdiag_t *diagnostics) {
	...
	diagnostics->rxPreamCount = (dwt_read32bitreg(RX_FINFO_ID)
			& RX_FINFO_RXPACC_MASK) >> RX_FINFO_RXPACC_SHIFT;
...
uint32 _dwt_otpprogword32(uint32 data, uint16 address) {
	...
	if ((rd_buf[0] & 0x02) != 0x02) {
		printf(
				"OTP PROG 32: ERROR VPP NOT OK, programming will fail. Are MR/MRA/MRB set?\n");
		return DWT_ERROR;
	}
	...
	if ((status & SYS_STATUS_LDEDONE) && (dw1000local.dblbuffon == 0))
		if ((status
			...
			if (dw1000local.sysCFGreg & SYS_CFG_RXAUTR) {
				dwt_write16bitoffsetreg(SYS_CTRL_ID, 0,
						(uint16) SYS_CTRL_RXENAB);
			} else {
...
#if (DEBUGx==1)
				printf("clear overrun \n");
#endif
				return;
			}

#if (DEBUGx==1)
			printf("status %02X%08X\n", 0, dwt_read32bitreg(SYS_STATUS_ID));
#endif
			{
				...
				if ((status & SYS_STATUS_AAT) //AAT bit is set (ACK has been requested)
						&& (((dw1000local.cdata.fctrl[0] & 0x20) == 0)
								|| (dw1000local.cdata.fctrl[0] == 0x02)) //but the data frame has it clear or it is an ACK frame
						) {
				...
				} else //double buffer
				{
					...
					if (dwt_checkoverrun()) {
						...
						dwt_writetodevice(SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET, 1,
								&hsrb); // we need to swap rx buffer status reg (write one to toggle internally)
					} else {
						dwt_forcetrxoff();
